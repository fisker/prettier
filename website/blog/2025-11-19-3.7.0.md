...
#### Improve emoji size measurement ([#17813](https://github.com/prettier/prettier/pull/17813) by [@seiyab](https://github.com/seiyab))
...
#### Infer TOML parser for Front Matter ([#17965](https://github.com/prettier/prettier/pull/17965) by [@kovsu](https://github.com/kovsu))

TOML Front Matter can be processed by the appropriate plugin if any. Works for HTML and CSS files too.

#### Fix strong emphasis format ([#18010](https://github.com/prettier/prettier/pull/18010) by [@yin1999](https://github.com/yin1999))

This is a supplementary fix for [#17143](https://github.com/prettier/prettier/pull/17143), where the original PR could not determine whether the contents on the left and right sides were both words.
...
#### Avoid creating `node_modules/.cache/` directory when `--cache` is not enabled ([#18124](https://github.com/prettier/prettier/pull/18124) by [@chiawendt](https://github.com/chiawendt))

Running `prettier .` without `--cache` no longer creates an empty `node_modules/.cache/` directory.

### API

#### Allow `plugin.parser.preprocess()` to return a `Promise` ([#17679](https://github.com/prettier/prettier/pull/17679) by [@fisker](https://github.com/fisker))

Aligns with [`plugin.printer.preprocess()`](https://prettier.io/docs/plugins#optional-preprocess), which allows returning a `Promise`.

We still suggest moving your async work into `plugin.parser.parse()`, which already allows returning a `Promise`. We may remove support for `plugin.parser.preprocess()` in the future.

#### Allow `AstPath#call()` to access property of nullish properties ([#17860](https://github.com/prettier/prettier/pull/17860) by [@fisker](https://github.com/fisker))
...
#### Allow plugin to provide an `estree` printer ([#18072](https://github.com/prettier/prettier/pull/18072) by [@fisker](https://github.com/fisker))

Previously, if a plugin wanted to make a plugin with `estree` printer that outputs different code based on the built-in one, the plugin needed to provide both `parsers` and `printers`. Prettier 3.7 allows a plugin to create a plugin that only provides an `estree` printer.
