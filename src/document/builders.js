...
let autoGeneratedGroupIdIndex = 0;
function addGroupIdIfNotExists(group) {
  if (group.id) {
    return;
  }

  autoGeneratedGroupIdIndex++;
  group.id = Symbol(`auto-generated-group-id-${autoGeneratedGroupIdIndex}`);
}

function getGroupId(options) {
  const { groupId, group } = options;

  if (groupId && group) {
    throw new Error('Can not use "groupId" and "group" together.');
  }
  ...
  if (group) {
    if (group.type !== DOC_TYPE_GROUP) {
      throw new TypeError('Unexpected "group"');
    }

    addGroupIdIfNotExists(group);
...
function ifBreak(breakContents, flatContents = "", options = {}) {
  ...
  const groupId = getGroupId(options);
 ...
 * @param {Doc} contents
 * @param {{ groupId?: symbol, group?: object, negate?: boolean }} options
 * @returns Doc
 */
function indentIfBreak(contents, options) {
  ...
  if (!groupId) {
    throw new Error('"options.group" or "options.groupId" is required.');
  }
